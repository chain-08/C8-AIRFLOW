...
SCHEDULE_CRON = "0 21 * * 1-5"  # 10:00 PM New York = 21:00 UTC
...
def create_dag():
    with DAG(
        ...
        start_date=datetime(2024, 1, 1, tzinfo=NY),
        schedule_interval=SCHEDULE_CRON,
        catchup=False,
        tags=["usd", "eod", "clickhouse"],
    ) as dag:

        fetch_task = PythonOperator(
            task_id="fetch_and_store_eod",
            python_callable=fetch_and_store_eod_data,
            op_kwargs={"retry_failed": False, "currency": "USD"},
        )
