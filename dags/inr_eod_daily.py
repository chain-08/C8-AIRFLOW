...
SCHEDULE_CRON = '0 15 * * 1-5'  # ✅ 8:30 PM IST (15:00 UTC)

TABLE_NAME = 'eod'
LOG_TABLE = 'eod_fetch_logs'
N_THREADS = 10
TIME_BUFFER = 5.5  # ✅ UTC +5.5 for IST
...
default_args = {
    'owner': 'airflow',
    'retries': 0,  # ✅ No retries, handled by second task
...
with DAG(
    ...
    def fetch_and_store_eod_data(retry_failed=False):
        ...
        # ✅ Date calculation based on local buffer
        ...
        if retry_failed:
            ...
            for t in failed_df['ticker']:
                parts = t.split('.')
                if len(parts) == 2:
                    symbols.append((parts[0], parts[1], 'Stock'))  # Assuming 'Stock' as type
        else:
    ...
    # ✅ Retry Task at 12:30 AM IST (Scheduled separately in Airflow UI as a second DAG or with time delay logic if needed)
